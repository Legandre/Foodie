...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val drawerToggle = ActionBarDrawerToggle(this,drawerLayout, com.example.foodie.R.string.open, com.example.foodie.R.string.close)
        ...
        Navigationview.setNavigationItemSelectedListener {
            ...
            when(it.itemId){
                ...
                com.example.foodie.R.id.MyProfile -> {
                    supportFragmentManager.beginTransaction().replace(com.example.foodie.R.id.frame, MyProfile()).addToBackStack("Profile").commit()
                    supportActionBar?.title = "My Profile"
                    drawerLayout.closeDrawers()
                }
                com.example.foodie.R.id.Fav_Restaurant -> {
                    supportFragmentManager.beginTransaction().replace(com.example.foodie.R.id.frame, Fav_Restaurant()).addToBackStack("FavRestaurant").commit()
                    supportActionBar?.title = "Favourite Restaurant "
                    drawerLayout.closeDrawers()
                }
                com.example.foodie.R.id.OrderHistory -> {
                    supportFragmentManager.beginTransaction().replace(com.example.foodie.R.id.frame, OrderHistory()).addToBackStack("OrderHistory").commit()
                    supportActionBar?.title = "Order History"
                    drawerLayout.closeDrawers()
                }
                com.example.foodie.R.id.Faqs -> {
                    supportFragmentManager.beginTransaction().replace(com.example.foodie.R.id.frame, FAQS()).addToBackStack("Faqs").commit()
    ...
    override fun onBackPressed() {
        when(supportFragmentManager.findFragmentById(com.example.foodie.R.id.frame)){
            !is Home -> openHome()
